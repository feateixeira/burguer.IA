...
const Orders = () => {
  ...
  // Tocar beep audível (curto) — tenta WebAudio; se falhar, usa Audio base64
  const playBeep = async () => {
    try {
      ...
      if (AudioContextRef) {
        ...
        // Two-tone alert: 1000Hz then 1200Hz
        mk(1000, 0, 0.25);
        mk(1200, 0.27, 0.35);
    ...
    try {
      ...
      audible.volume = 1; await audible.play().catch(() => {});
    } catch {}
  };
  ...
  const loadOrders = async (opts?: { background?: boolean }) => {
    try {
      ...
      // Detect new pending site orders
      const newPendingSite = (data || []).filter(o =>
        (o.channel === "online" || o.origin === "site" || o.source_domain) &&
        (o.status === "pending" || o.status === "preparing")
      );
      const currentIds = new Set(newPendingSite.map(o => o.id));
      ...
      if (hasNew) {
        await playBeep();
        toast.info("Novo pedido do site chegou!");
        if (!notificationPermissionAskedRef.current && typeof Notification !== 'undefined' && Notification.permission === 'default') {
          try { await Notification.requestPermission(); } catch {}
          notificationPermissionAskedRef.current = true;
        }
        if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
          try { new Notification('Novo pedido', { body: 'Chegou um novo pedido do site.' }); } catch {}
        }
      }
    } catch (error) {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    try {
      // Prefer secure RPC to handle cascades and RLS safely
      const { error: rpcError } = await (supabase as any).rpc('delete_order_cascade', {
        p_order_id: orderId,
        p_establishment_id: establishment.id
      });

      if (rpcError) {
        // Fallback: legacy client-side deletes
        ...
        // Also attempt to delete PIX payments if exist
        await supabase
          .from("pix_payments")
          .delete()
          .eq("order_id", orderId);
