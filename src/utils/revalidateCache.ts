...
export async function revalidateCache(paths: string[], establishmentSlug?: string): Promise<void> {
  try {
    ...
    if (!session) {
      console.warn('No session found, skipping cache revalidation');
      return;
    }
    ...
    if (response.error) {
      console.error('Cache revalidation error:', response.error);
    } else {
      console.log('✅ Cache revalidated for paths:', fullPaths);
    }
  } catch (error) {
    // Fail silently - revalidation should not break the main flow
    console.error('Error in cache revalidation (non-critical):', error);
  }
}
