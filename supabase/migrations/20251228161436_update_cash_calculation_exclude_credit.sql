-- ============================================
-- ATUALIZAR FUNÇÃO DE CÁLCULO DE CAIXA: EXCLUIR FIADO NÃO RECEBIDO
-- ============================================
...
BEGIN
  ...
  -- IMPORTANTE: Excluir pedidos fiado não recebidos do cálculo
  -- Incluir apenas:
  -- 1. Pedidos normais (não fiado) criados no período da sessão
  -- 2. Pedidos fiado recebidos no período da sessão (usar credit_received_at)
  ...
  WHERE o.establishment_id = v_session.establishment_id
    ...
    -- Filtrar fiado: apenas pedidos normais ou fiado recebidos
    AND (
      ...
      (o.is_credit_sale = true AND o.credit_received_at IS NOT NULL 
        ...
        -- Para fiado recebido, usar o valor total com juros
        AND o.payment_method = 'dinheiro' OR o.payment_method = 'cash')
    )
